    ...
    for (let i = 0; i < strings.length; i++) {
        ...
        if (i < args.length) {
            if (typeof args[i] == "function") {
                ...
                if (["oninput", "onchange", "onsubmit", "onclick", "onmouseover", "onmouseout", "onmousemove", "onmousedown", "onmouseup", "onkeydown", "onkeyup", "onkeypress", "ondblclick", "onload", "onunload"].some(substring => strings[i].includes(substring))){                                                              result += `function_store[${store_function(args[i])}](event)`
                } else {
...
function rerender(component_ref_key, props){
    ...
    let primaryJsonData;
    if (!props == undefined) {
        primaryJsonData = data_store[element.getAttribute("data-key")];
    } else {
